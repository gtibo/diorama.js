...
class Body {
	...
	getTileCollisionData(x, y) {
		...
		let tile_id = this.world.getTile(1, tile_pos.x, tile_pos.y);
		if (this.world.terrain.tileset.tileproperties[tile_id] !== undefined
			&& this.world.terrain.tileset.tileproperties[tile_id].collision === true) {
			let neighbors = [
				{
					x: tile_pos.x - 1,
					y: tile_pos.y
				...
				let n_id = this.world.getTile(1, tile.x, tile.y);
				if (this.world.terrain.tileset.tileproperties[n_id] !== undefined
					&& this.world.terrain.tileset.tileproperties[n_id].collision === true) {
							return true;
				} else {
					return false;
				}
	...
	mapCollision(){
		...
		let top_right = this.getTileCollisionData(tX + this.size.x, tY);
		let bottom_left = this.getTileCollisionData(tX, tY + this.size.y);
		let bottom_right = this.getTileCollisionData(tX + this.size.x, tY + this.size.y);
