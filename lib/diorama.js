class Diorama {
	constructor(parameters) {
		...
		this.debug = false;
		// canvas
		this.background_color = parameters.background || "#000";
		...
		this.fonts = {};
		// Maps
		this.tile_size = 16 || parameters.tile_size;
		this.maps = {};
		// Box system
		this.boxes = {};
		// Game loop Data
	...
	startScene(scene_name) {
		...
		if (scene_name == undefined) scene_name = Object.keys(game.scenes)[0];
	...
	getTile(layer_id, x, y) {
		if (x < 0 || x > this.terrain.layers[layer_id].width-1) return false;
		if (y < 0 || y > this.terrain.layers[layer_id].height-1) return false;
		if(typeof layer_id !== "number"){
			layer_id = this.terrain.layers.find(layer=>{
				return layer.name === layer_name;
			});
		}
		return this.terrain.layers[layer_id].data[y][x];
	}
